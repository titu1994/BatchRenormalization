...
def __conv2_block(input, k=1, dropout=0.0, mode=0):
    init = input
...
def __conv3_block(input, k=1, dropout=0.0, mode=0):
    init = input
...
def __create_wide_residual_network(nb_classes, img_input, include_top, depth=28, width=8, dropout=0.0, mode=0):
    ...
    x = __conv1_block(img_input, mode)
    nb_conv = 4

    for i in range(N):
        x = __conv2_block(x, width, dropout, mode)
        nb_conv += 2

    x = MaxPooling2D((2, 2))(x)

    for i in range(N):
        x = __conv3_block(x, width, dropout, mode)
        nb_conv += 2

    x = MaxPooling2D((2, 2))(x)

    for i in range(N):
        x = ___conv4_block(x, width, dropout, mode)
        nb_conv += 2
